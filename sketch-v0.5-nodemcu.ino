...
const int microSD = D8; // chip esclavo microSD puerto SPI - en el uno pin 4 - en el esp8266 nodemcu 1.0 v3 pin D8 gpio15
...
if (!SD.begin(microSD)) {       // sino inicia el modulo lector sd
...
/* if (!dht.begin()) {       // sino inicializa modulo con direccion I2C definida previamente
  ...
  dht.begin();
  Serial.println(F("Modulo DHT iniciado correctamente")); // muestra mensaje de confirmacion
...
void loop() {
  ...
  if (millis() >= lecturaMillis + 5000) {
    ...
    if (millis() >= lecturaMillis + 500) {
      registrarTemperaturaDHT();
    }
    if (millis() >= lecturaMillis + 750) {
      registrarTemperaturaBMP();
    }
